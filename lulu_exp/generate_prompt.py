...
def get_task_prompt( query_str, if_english = False):
    if not if_english:
        return f"""我正在编写一个简易的早教游戏。请参考系统函数说明中的函数，以及对应例子。编写一个Query = "{query_str}"相关的游戏。"""
    else:
...
整个游戏的画面是800 * 500的，并且提供以下函数

## spawn_card

spawn_card( text ) 或 spawn_card( text ,x, y ) 
根据text对应的卡片，在x,y中心位置生成卡片，如spawn_card("苹果")或者spawn_card("一个盘子",400,300)
...
set_target( answer_text, x, y ) 在x,y中心位置生成一个虚线框，如果用户将答案对应的card拖动到框中，则会播放correct_text

其中可选参数为
answer_text 默认的答案卡片
x,y 中心位置，必要， 如果题目只有一行一般y = 250，有两行时y = 180和 320 以此类推
scale 默认为1
...
```python
spawn_card( "你好" ) # 自动生成在候选区
set_target( "你好", 400, 300, correct_text = "成功啦" )
```"""
...
根据Query的需求，参考Reference中的例子，进行python程序的编写 

Let's think it step-by-step 并输出为json格式，包含两个字段
- analysis 分析具体的实现思路
- python_code 整段python代码的具体实现"""
    else:
        return f"""# Goal
...
According to the requirements of Query, refer to the example in Reference, and write a python program.
